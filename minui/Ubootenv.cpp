...
int Ubootenv::init() {
    ...
    const char *BLOCK_ENV = "/dev/block/by-name/env";//normally use that
...
//#endif
    ...
    if (prefix[0] == 0) {
        ...
        printf("[ubootenv] set property ro.ubootenv.variable.prefix: %s\n", prefix);
        property_set("ro.ubootenv.variable.prefix", prefix);
    }

    if (strlen(prefix) > 16) {
        printf("[ubootenv] Cannot r/w ubootenv variables - prefix length > 16.\n");
        return -4;
    }

    sprintf(PROFIX_UBOOTENV_VAR, "%s.", prefix);
    printf("[ubootenv] ubootenv variable prefix is: %s\n", prefix);
#endif
